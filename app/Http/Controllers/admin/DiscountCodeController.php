...
class DiscountCodeController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if($validator->passes()){
            ...
            if(!empty($request->starts_at)){
                ...
                if($startAt->lte($now) == true){
                    return response()->json([
                        'status' => false,
                        'errors' => ['starts_at' => 'Start date cant be less than current date time']
                    ]);
                }
            }
            ...
            if(!empty($request->starts_at) && !empty($request->expires_at)){
                $expiredAt = Carbon::createFromFormat('Y-m-d H:i:s',$request->expires_at);
                $startAt = Carbon::createFromFormat('Y-m-d H:i:s',$request->starts_at);

                if($expiredAt->gt($startAt) == false){
                    return response()->json([
                        'status' => false,
                        'errors' => ['expires_at' => 'Expired date must be greater than start date']
                    ]);
                }
            }
    ...
    public function edit(Request $request, $id){
        ...
        if($coupon == null){
            session()->flash('erros','Record not found');
